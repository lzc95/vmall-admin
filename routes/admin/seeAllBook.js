...
module.exports=function(){
	var router = express.Router();
     ...
     router.get('/',(req,res)=>{   	 
     	  db.query(`SELECT shop_table.shop_name,book_table.* FROM shop_table,
               book_table WHERE shop_table.shop_id=book_table.shop_id`,(err,data1)=>{
     	  	  if(err){
     	  	  	console.error(err);
     	  	  }
     	  	  else{
                    db.query(`SELECT cName FROM category`,(err,data2)=>{
                       if(err){
                         console.error(err);
                       }
                       else{
                       res.render('admin/seeAllBook.ejs',{adminname:req.session['adminname'],bookData:data1,
                       sortData:data2})
                       }
                    })
     	  	  }
